...
snakemake -p -j $MAXJOBS --cluster-config cfg/cluster.json --rerun-incomplete --jobname "msi-{rulename}-{jobid}" --cluster "sbatch -A {cluster.account} -p {cluster.partition} --ntasks={cluster.n}  -t {cluster.time} --mem={cluster.memory} --output=log/slurm-%j.out --error=log/slurm-%j.out"
