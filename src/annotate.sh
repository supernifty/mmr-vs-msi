...
#################
# - filter vcf on mmr exons
bedtools intersect -a $INPUT -b $REGIONS -header -wa -u > ${OUTPUT}.tmp

#################
# - run vep

echo "starting vep for $INPUT at $(date)"

VEP_INPUT=${OUTPUT}.tmp

$VEPPATH/vep --cache --refseq --offline --dir_cache  $CACHE --fasta $REFERENCE \
    -i "$VEP_INPUT" --sift b --polyphen b --symbol --numbers --biotype --total_length --hgvs \
...
echo "finished vep for $INPUT at $(date)"
